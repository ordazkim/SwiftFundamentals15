 ...
 */
let nameInCaps = "Kimberly M. Ordaz"
let name = "kimberly m. ordaz"
if name.uppercased() == nameInCaps{
 ...
 */
let lowercasedNameInCaps = nameInCaps.lowercased()
let lowercasedName = name.lowercased()
if lowercasedNameInCaps == lowercasedName{
    print("\"\(lowercasedNameInCaps)\"and  \"\(lowercasedName)\"are the same")
} else {
    print("\"\(lowercasedNameInCaps)\" and \"\(lowercasedName)\" are not the same")
}
...
if textToSearchThrough.range(of :textToSearchFor, options: .caseInsensitive) != nil{
    print("I found it!")
}

//:  Print to the console the number of characters in your name by using the `count` property on `name`.
let Name = "Kimberly"
print ("The number of characters in the name \(Name) is \(Name.count)")
/*:
